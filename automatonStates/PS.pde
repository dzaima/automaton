...
/*/
automatonStates thisa = this;
void settings() {
  noSmooth();
  size(900, 800);
}
static final boolean phone = false;
static byte usedState = 1;
void mouseActions() {
  ...
  if (mousePressed && mouseButton == LEFT && !playing) {
    ...
    int py = (int) (mouseY/zm + dsy);
    if (mousePressed && !pmp) {
      drawOn = board.get(px, py)!=usedState;
    }
    if (drawOn) board.set(px, py, usedState);
    else board.set(px, py, (byte)0);
  }
}
void mouseWheel(MouseEvent e) {
  if (e.isControlDown()) {
    usedState-= e.getCount();
    if (usedState >= COLORS.length) usedState = (byte)(COLORS.length-1);
    if (usedState < 1) usedState = 1;
  } else {
